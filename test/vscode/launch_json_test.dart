...
void main() {
  ...
  group('.vscode/launch.json', () {
    test('pathes in launch.json', () {
      ...
      // Ensure there is a configuration for executing bin/testproject_gg_3.dart
      final configurations =
          parsedLaunchJson['configurations'] as List<dynamic>;

      final testprojectGg3 = configurations.firstWhere(
        (dynamic configuration) =>
            configuration['name'].toString() == 'testproject_gg_3.dart',
      );

      expect(
        testprojectGg3,
        isNotNull,
        reason: 'Item for executing bin/testproject_gg_3.dart not found',
      );

      expect(testprojectGg3['name'], 'testproject_gg_3.dart');
      expect(testprojectGg3['type'], 'dart');
      expect(testprojectGg3['request'], 'launch');
